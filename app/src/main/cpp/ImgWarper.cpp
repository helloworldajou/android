...
cv::Mat ImgWarper::warp(cv::Mat _img, vector<cv::Point_<double> > source_points, vector<cv::Point_<double> > dest_points){
    ...
    cout << (grid_rows) * (grid_cols) << " " << grid.size()<<endl;

    
    clock_t after_deformation = clock();
    ...
    clock_t warp_finished = clock();

    double deformation_time = double(after_deformation - warp_started) / CLOCKS_PER_SEC;
    double interpolation_time = double(warp_finished - after_deformation) / CLOCKS_PER_SEC;
    double warping_time = double(warp_finished - warp_started) / CLOCKS_PER_SEC;
    cout << "Deform: " << deformation_time << " Interpolation: " << interpolation_time << std::endl;
